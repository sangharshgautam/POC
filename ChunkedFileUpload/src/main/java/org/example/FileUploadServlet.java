		...
		try {
			...
			while (i.hasNext()) {
				...
				if (!fi.isFormField()) {
					...
					final Integer chunkName = Integer.parseInt(fi.getName());
					...
					if(parts !=null){
						...
						if(parts.size() == totalChunks){
							...
							for(int index = 0; index<parts.size();index++){
								FileItem chunkFileItem  = parts.get(index);
								
								InputStream is = chunkFileItem.getInputStream();
					...
					out.println("Uploaded Filename: " + chunkName + "<br>");
				}
			}
			out.println("</body>");
			out.println("</html>");
